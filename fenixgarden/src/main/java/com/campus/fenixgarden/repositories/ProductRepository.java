...
public interface ProductRepository extends JpaRepository<Product, String> {
    //1
    @Query("SELECT p.productCode, p.name, p.gamaProduct.gama, p.amountInStock, p.salePrice FROM Product p WHERE p.gamaProduct.gama = ?1 AND p.amountInStock > 100 ORDER BY p.salePrice DESC")
    List<Object> findOrnamentalProductsInStockGreaterThan100(String gama);
    //2
    @Query("SELECT p.productCode, p.name, p.gamaProduct.gama, p.dimensions FROM Product p WHERE p.productCode NOT IN (SELECT DISTINCT d.product.productCode FROM OrderDetail d)")
    List<Object> findProductsNeverOrdered();
    //3
